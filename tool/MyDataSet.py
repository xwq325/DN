...
rot_deal = [cv2.ROTATE_90_CLOCKWISE, cv2.ROTATE_180, cv2.ROTATE_90_COUNTERCLOCKWISE]
...
class Art_nosie_Dataset(Dataset):  # 添加人工噪声
    ...
    def __init__(self, args, data_dir, mode='train', ori_image_size=True):
        ...
        self.mode = mode
        if mode == 'train':
            self.data_list = self.train_data_generator(data_dir)  # patches 的集合
        else:  # mode == test
            self.data_list, self.name_list = self.test_data_generator(data_dir)
            # self.data_lsit = self.test_data_generator(data_dir)

    def __getitem__(self, index):
        clean_img = self.data_list[index]
        # print(np.array(clean_img).shape)
    ...
    def gen_patches(self, img, patch_size=48, n=128, aug=True, aug_plus=False):
        ...
        if ih < ip:
            # print("ih:{}<ip:{}".format(ih, ip))
            return patches
        elif iw < ip:
            # print("iw:{}<ip:{}".format(iw, ip))
            return patches

        for _ in range(0, n):  # 一张图片产生n个patches
            iy = random.randrange(0, ih - ip + 1)  # 返回 0 到 ih - ip + 1 之间的一个整数
    ...
    def train_data_generator(self, data_dir):
        ...
        for img_name in filelist:
            ...
            # 1张图片产生Patches   48x48 256张
            if self.args.no_augment:
                augs = False
            else:
                augs = True
            patches = self.gen_patches(img, patch_size=self.patch_size, n=self.n_patches,
                                       aug=augs,
        ...
        img_list = list(chain(*img_list))  # (x张图, 256, 48, 48, 3)  ----->(X张x256, 48, 48, 3)
    ...
    def test_data_generator(self, data_dir):
        ...
        # os.listdir() 方法用于返回指定的文件夹包含的文件或文件夹的名字的列表。这个列表以字母顺序
        if not os.path.exists(data_dir):  # 没有该文件夹，则创建该文件夹
            os.makedirs(data_dir)
            print("Make the dir:{}".format(data_dir))
        ...
        for img_name in filelist:
            ...
            # patches = utils_image.patches_from_image(img, p_size=48, p_overlap=0, p_max=48)
            # print('=====================================')
        ...
        # name_len = np.array(img_list).shape
        # name_list = [filelist[i] for i in range(name_len[0]) for j in range(name_len[1])]
        # img_list = list(chain(*img_list))  # (x张图, 256, 48, 48, 3)  ----->(X张x256, 48, 48, 3)
        # print("np.array(img_list).shape", np.array(img_list).shape)
        # print("np.array(name_list).shape", np.array(name_list).shape)
