...
def join(image_dir):
    ...
    while i >= 0:
        ...
        i = i - 1
    # 创建一个新的白色背景图像
    new_image = Image.new('RGB', (4 * width + 30, 2 * height + 140), 'white')
    ...
    new_image.paste(images[4], (0, 70 + height))
    new_image.paste(images[5], (10 + width, 70 + height))
    new_image.paste(images[6], (20 + 2 * width, 70 + height))
    new_image.paste(images[7], (30 + 3 * width, 70 + height))

    draw = ImageDraw.Draw(new_image)
    font = ImageFont.truetype(font="draw/arial.ttf", size=50)

    x_ = [0.5 * width - 25, 10 + width + 0.5 * width - 25, 20 + 2 * width + 0.5 * width - 25,
          30 + 3 * width + 0.5 * width - 25]
    for i in range(8):
        if i <= 3:
            x = x_[i]
            y = height + 5
            ...
            x = x_[i - 4]
            y = 2 * height + 70 + 5
        draw.text((x, y), "({})".format(ch[i]), font=font, fill=(0, 0, 0))
...
def enlarge_drawing(x, y, image_name, savepath, image, right=False, save=False, scale=0.6):
    ...
    ax.add_patch(plt.Rectangle((x, y), width, hight, color="blue", fill=False, linewidth=3))
    ...
    save_path = os.path.join(savepath, image_name[:-3] + "png")
